...
| **FR-08** | เข้าสู่ระบบด้วย Google — ตรวจ ID token **ที่เซิร์ฟเวอร์** (`aud` = client id ของระบบ, `iss` = accounts.google.com, `hd` = โดเมนคณะ, `email_verified` = true) และผูกบัญชีด้วย `sub` ไม่ใช่อีเมล · อีเมลที่ยังไม่มีบัญชีสร้างเป็นผู้สอนที่ยืนยันแล้ว *(ใหม่ 2026-09-14 · D6)* | MUST | Login |
